...
extension ItemColorExtension on ItemColor {
  ...
  Color onColor(BuildContext context) {
    ...
    return ThemeData.estimateBrightnessForColor(bgColor) == Brightness.dark
        ? AppColors.base // Use light color from your theme
        : AppColors.text; // Use dark color from your theme
  }
}
...
class Task {
  ...
  final bool isHibernated; // FIX: Added missing field
  ...
  Task({
    ...
    this.parentId,
    this.isHibernated = false, // FIX: Added to constructor
    this.children = const [],
    this.tags = const [],
    this.progress = 0.0,
  });

  /// FIX: Added the missing copyWith method, required by the TaskController.
  Task copyWith({
    String? id,
    String? userId,
    DateTime? createdAt,
    ...
    return Task(
      ...
      isHibernated: isHibernated ?? this.isHibernated,
  ...
  factory Task.fromJson(Map<String, dynamic> json) {
    return Task(
      ...
      isHibernated: json['is_hibernated'] ?? false, // FIX: Added parsing for hibernate status
    );
  }

  Map<String, dynamic> toJson() {
    return {
      // id, userId, and createdAt are handled by Supabase, not sent in updates
      'title': title,
      'content': content,
      ...
      'parent_id': parentId,
      'is_hibernated': isHibernated, // FIX: Added for saving to Supabase
      // children, tags, and progress are typically not sent this way
    };
  }
}
